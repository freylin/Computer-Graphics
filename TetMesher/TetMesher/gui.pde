void keyPressed() 
  ...
  if(key=='o') {useOctree = !useOctree;}
  if(key=='i') {stepToShow++; if(stepToShow == maxStep) stepToShow = 0;}
  if(key=='0') {showPivotBall = !showPivotBall;}
  if(key=='b') {showBallPath = !showBallPath;}
  if(key=='m') {showMesh = !showMesh; }
  if(key=='y') {showVertexTypes = !showVertexTypes;}
  if(key=='e') {showInnerEdges = !showInnerEdges; showBorderEdges = !showBorderEdges;}
  if(key=='v') {showVertices = !showVertices; }
  if(key=='r') {useRadius = !useRadius;}
...
void displayHeader()  // Displays title and authors face on screen
    ...
    fill(white); image(YaohongPix, width-YaohongPix.width/2,25,YaohongPix.width/2,YaohongPix.height/2); 
    fill(white); image(JinPix, width-YaohongPix.width/2-JinPix.width/2 - 10,25,JinPix.width/2,JinPix.height/2); 
    }
...
String title ="Lattice Maker", name ="Jin Lin, Yaohong Wu",
