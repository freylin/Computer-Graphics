...
//  if(key=='`') picking=true; 
  ...
  if(key=='r') showSkater = !showSkater;
  if(key=='g') useCurvature = !useCurvature;
  if(key=='o') useOffsetCurve = !useOffsetCurve;
  if(key=='1') constStepLength = !constStepLength;
  if(key=='2') doRetrofitting = !doRetrofitting;
  if(key=='+') {if(stepLength < 96) stepLength += 5;}
  if(key=='-') {if(stepLength > 9) stepLength -= 5;}
...
String title ="CS6491-2017 Project 3: Runner", name ="Yaohong Wu, Jin Lin",
